...
def drivebase_control(): 
    if (abs(controller.axisA.position()) > 10 or abs(controller.axisB.position()) > 20): 
        LeftBase.spin(FORWARD,controller.axisB.position() - controller.axisA.position() * 0.7,PERCENT)
        RightBase.spin(FORWARD,controller.axisB.position() + controller.axisA.position() * 0.7, PERCENT)
        
    else: 
        LeftBase.stop()
        RightBase.stop()

    if (abs(controller.axisC.position()) > 10 or abs(controller.axisD.position()) > 20): 
        LeftBase.spin(FORWARD,controller.axisD.position() - controller.axisC.position() * 0.7,PERCENT)
        RightBase.spin(FORWARD,controller.axisC.position() + controller.axisD.position() * 0.7, PERCENT)
...
def BlueTrigger(): 
    Bluetower.spin_for(FORWARD, 120, DEGREES,wait=True)
    wait(0, SECONDS)
    Bluetower.spin_to_position(0, DEGREES)
