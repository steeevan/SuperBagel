...
def drivebase_control(): 
    if (abs(controller.axisC.position()) > 10 or abs(controller.axisA.position()) > 20): 
        LeftBase.spin(FORWARD,controller.axisC.position() - controller.axisA.position() * 0.7,PERCENT)
        RightBase.spin(FORWARD,controller.axisC.position() + controller.axisA.position() * 0.7, PERCENT)
    else: 
        LeftBase.stop()
        RightBase.stop()
...
def blue_control(): 
    controller.buttonFDown.pressed(ArmExtention)
    controller.buttonEDown.pressed(DiscDrop)
